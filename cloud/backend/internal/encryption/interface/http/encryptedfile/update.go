// cloud/backend/internal/encryption/interface/http/encryptedfile/update.go
...
import (
	...
	svc "github.com/Maple-Open-Tech/monorepo/cloud/backend/internal/encryption/service/encryptedfile"
...
func (h *UpdateEncryptedFileHandler) Pattern() string {
	return "PUT /api/v1/encrypted-files/{id}"
}
...
func (h *UpdateEncryptedFileHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	var file *http.File
	fileHeader, err := r.FormFile("encrypted_content")
	if err == nil && fileHeader != nil {
		f, err := fileHeader.Open()
		if err != nil {
			h.logger.Error("Failed to open file from form", zap.Error(err))
			httperror.ResponseError(w, httperror.NewForBadRequestWithSingleField("encrypted_content", "Could not process file content"))
			return
		}
		file = &f
		defer (*file).Close()
